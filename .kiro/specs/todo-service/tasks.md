...
- [x] 5.1 ユーザー設定 UI の実装 (#16)
  ...
  - ユーザー名・パスワード更新フォーム実装
  - Supabase Storage を使用したアバター画像アップロード機能の実装
  - アカウント削除確認と実行アクション追加
...
- [ ] 8. UI/UX の拡張 (#29)
- [x] 8.1 ダークモードへの対応
  - Tailwind CSS を使用したダークモードスタイルの適用
  - システム設定との同期および手動切り替えトグルの実装
  - Profile Settings に設定を追加
- [ ] 8.2 多言語化 (英語・日本語) 切り替えへの対応
  - `next-intl` を使用した翻訳ファイルの整備
  - 言語切り替えセレクターの実装とルーティングの調整
  - Profile Settings に設定を追加
- [ ] 8.3 サービス名称の変更
  - サービス名称を `todo-service` から `todo-kanban` に変更

- [ ] 9. ドキュメント整備 (#30)
- [ ] 9.1 リポジトリ探索と README.md の更新
  - プロジェクト全体の構成を再確認
  - 構築手順や機能概要を網羅した最新の `README.md` へ更新

- [ ] 10. リファクタリングと最適化 (#31)
- [ ] 10.1 環境変数ファイルの統一管理
  - `frontend/`, `backend/` に散在する `.env.local`, `.env.test` をルートの `.env` 類に集約
  - 各アプリケーションの読み込み設定をルート参照に修正
- [ ] 10.2 実装要件の充足状況の最終調査
  - `tasks.md` の各項目と現在のソースコードを照らし合わせ、未実装や不備がないか確認
  - 必要に応じて微修正または追加タスクの起票
